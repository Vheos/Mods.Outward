...
public class Needs : AMod, IDelayedInit
{
	...
	private static readonly Item[] WATERS = new[]
	{
		"Clean Water".ToItemPrefab(),
		"River Water".ToItemPrefab(),
		"Salt Water".ToItemPrefab(),
		"Rancid Water".ToItemPrefab(),
		"Leyline Water".ToItemPrefab(),
		"Sparkling Water".ToItemPrefab(),
		"Healing Water".ToItemPrefab(),
	};
	...
	private static ModSetting<int> _drinkValuesWaters, _drinkValuesPotions, _drinkValuesOther;
	...
	protected override void Initialize()
	{
		...
		_drinkValuesWaters = CreateSetting(nameof(_drinkValuesWaters), 30, IntRange(0, 100));
		...
		foreach (var data in NEEDS_DATA)
		{
			...
			using (Indent)
			{
				...
				if (data.Need == Need.Drink)
				{
					...
					using (Indent)
					{
						_drinkValuesWaters.Format("Waters", _overrideDrinkValues);
						_drinkValuesWaters.Description = "all kinds of water";
	...
	private static void UpdateDrinkValues()
	{
		foreach (var ingestibleByID in Prefabs.IngestiblesByID)
		{
			...
			float drinkValue = ingestible.IsContainedIn(WATERS) ? _drinkValuesWaters
				: ingestible.IsContainedIn(OTHER_DRINKS) ? _drinkValuesOther
				: _drinkValuesPotions;
