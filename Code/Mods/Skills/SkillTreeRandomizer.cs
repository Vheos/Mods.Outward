...
public class SkillTreeRandomizer : AMod, IDelayedInit
{
	...
	private const string REROLL_STATS_FILE_NAME = "SkillTreeRandomizerStats";
	...
	private static ModSetting<bool> _treatSpecialistAsAdvanced;
	...
	protected override void Initialize()
	{
		...
		_treatSpecialistAsAdvanced = CreateSetting(nameof(_treatSpecialistAsAdvanced), false);
	...
	protected override void SetFormatting()
	{
		...
		_treatWeaponMasterAsAdvanced.Description = "Weapon Master's skills will be treated as advanced instead of basic\n" +
												   "(which they technically are as the skill tree doesn't have a breakthrough)";
		_treatSpecialistAsAdvanced.Format("Treat Specialist as advanced", _theThreeBrothersInput, TheThreeBrothersInput.Specialist);
		_treatSpecialistAsAdvanced.Description = "Specialist's skills will be treated as advanced instead of basic\n" +
											     "(which they technically are as the skill tree doesn't have a breakthrough)";
		_seed.Format("Seed");
	...
	private static IEnumerable<Trait<BaseSkillSlot>> GetTraits(IEnumerable<SkillSchool> trees)
	{
		...
		if (traits.HasFlag(EqualizedTraits.TypesPerLevel))
		{
			yield return new Trait<BaseSkillSlot>("Active Basic", slot => GetType(slot) == SlotType.Active && GetLevel(slot) == SlotLevel.Basic);
			yield return new Trait<BaseSkillSlot>("Active Advanced", slot => GetType(slot) == SlotType.Active && GetLevel(slot) == SlotLevel.Advanced);
			yield return new Trait<BaseSkillSlot>("Passive Basic", slot => GetType(slot) == SlotType.Passive && GetLevel(slot) == SlotLevel.Basic);
			yield return new Trait<BaseSkillSlot>("Passive Advanced", slot => GetType(slot) == SlotType.Passive && GetLevel(slot) == SlotLevel.Advanced);
		}
	...
	private static void RandomizeSkills()
	{
		...
		{
			LogResultsToFile(REROLL_STATS_FILE_NAME, equalizer.GetResultsAsString());

			var sb = new StringBuilder();
			sb.AppendLine("Tree\tSlotType\tSkills");

			foreach (var slot in outputTrees.SelectMany(tree => tree.SkillSlots))
			{
				sb.Append($"{slot.ParentBranch.ParentTree.Name}");
				if(slot is SkillSlotFork skillChoice)
				{
					sb.Append($"\tChoice");
					foreach (var choiceSlot in skillChoice.m_skillsToChooseFrom)
						sb.Append($"\t{choiceSlot.Skill.name}");
				}
				else if (slot is SkillSlot skillSingle)
				{
					sb.Append($"\tSingle\t{skillSingle.Skill.Name}");
				}

				sb.AppendLine();
			}

			LogResultsToFile(REROLL_STATS_FILE_NAME + " (Spoilers)", sb.ToString());
		}
	}
	private static void LogResultsToFile(string fileName, string results)
	=> System.IO.File.WriteAllText(Utils.PluginFolderPath + fileName + ".txt", results);
	...
	private static BaseSkillSlot CopySlot(BaseSkillSlot slot)
	{
		...
		if (newSlot is SkillSlotFork fork)
			foreach (var forkSlot in fork.SkillsToChooseFrom)
				forkSlot.RequiredSkillSlot = null;
	...
	private static SlotLevel GetLevel(BaseSkillSlot slot)
		=> _treatWeaponMasterAsAdvanced && GetVanillaTree(slot) == FlagToSkillTree(TheThreeBrothersInput.WeaponMaster, true)
		|| _treatSpecialistAsAdvanced && GetVanillaTree(slot) == FlagToSkillTree(TheThreeBrothersInput.Specialist, true)
		? SlotLevel.Advanced
